...
Each statute sheet opens with **Elements** — the operative clauses of the exact subsection the record cites, quoted word for word from the retrieved text, with **Important exceptions** listed separately. Nothing there is written by the application: clauses are cut on statutory punctuation only, so qualifiers survive by construction, and a test asserts every displayed clause is a verbatim substring of what ILGA served. Within each clause the qualifiers an officer has to satisfy — `knowingly`, `more than`, `without`, `unless` — are emphasised; emphasis is presentation only, and a test asserts the rendered text is byte-identical to the statute.

Plain language exists solely to make search match. It never renders, so an alias cannot be read as statutory wording.

Where a nested provision is not published separately the sheet says so and falls back to the enclosing subsection rather than passing a lead-in off as the cited element; where a record cites no subsection, the panel is labelled the section's opening provision. Omitted clauses are counted on screen rather than dropped silently. Every record carries a **Report an issue** action that opens a prefilled report for the officers reviewing the content in the field.
